function [] = stressPlot(L,d_Comp,H,xy_bar,D,comp)
% PLOT STRESS CONTOURS OF EACH ELEMENT
% Shape functions
syms xi real
syms eta real
sh(1) = 1/4 * (1-xi)*(1-eta);
...
d1 = L*disp;
strains = H*d1;
strains = D*strains; % Strains var name used for stresses
...
Number = 100;
if comp == 1
    strain_xx = subs(strains(1),{eta,xi},{eta_2,xi_2});
    contourf(xx,yy,strain_xx,Number,'LineColor','none')
    title('Contours for \sigma_{11}')
    xlabel('X (meters)')
    ylabel('Y (meters)')
end

if comp == 2
    strain_yy = subs(strains(2),{eta,xi},{eta_2,xi_2});
    contourf(xx,yy,strain_yy,Number,'LineColor','none')
    title('Contours for \sigma_{22}')
    xlabel('X (meters)')
    ylabel('Y (meters)')
